#line 1 "/Users/sumioka_air/USD/pxr/usd/lib/usdHydra/texture.h"
//
...
public:
    ...
    /// Construct a UsdHydraTexture on UsdPrim \p prim .
    /// Equivalent to UsdHydraTexture::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit UsdHydraTexture(const UsdPrim& prim=UsdPrim())
        : UsdShadeShader(prim)
    {
    }

    /// Construct a UsdHydraTexture on the prim held by \p schemaObj .
    /// Should be preferred over UsdHydraTexture(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit UsdHydraTexture(const UsdSchemaBase& schemaObj)
